...
param_grid = {
    ...
    'model__colsample_bytree': [0.7, 0.8, 0.9, 1],
    'model__learning_rate': [0.01, 0.05, 0.1, 0.2, 0.4],
...
grid_xgb = GridSearchCV(pipeline, param_grid, cv=5, n_jobs=-1, scoring='accuracy', verbose=0, return_train_score=True)
...
# %%
# best result xgb
# {'model__booster': 'dart', 'model__colsample_bytree': 0.9, 'model__learning_rate': 0.2, 'model__max_depth': 8,
#  'model__n_estimators': 10, 'model__objective': 'binary:logistic', 'model__random_state': 1337,
#  'model__verbosity': 0}
#
# grid_xgb = xgb.XGBClassifier(booster='dart', colsample_bytree=0.9, learning_rate=0.2, max_depth=8, n_estimators=10,
#                              objective='binary:logistic', random_state=RAND_SEED)
# grid_xgb.fit(X_train, y_train)
...
xgb_predictions = grid_xgb.predict(scaled_features_df)

xgb_predictions_titanic = pd.DataFrame({'PassengerId': titan_df_valid['PassengerId'], 'Survived': xgb_predictions})
xgb_predictions_titanic = xgb_predictions_titanic.astype(int)
xgb_predictions_titanic.to_csv(data_folder / 'submission_titanic_dahernes.csv', index=False)
print(xgb_predictions_titanic)
